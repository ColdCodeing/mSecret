package com.mm.secretapp.utils
...
val RCON = arrayOf(
        Word(RCON_INIT[0],0x00, 0x00, 0x00), Word(RCON_INIT[1],0x00, 0x00, 0x00),
        Word(RCON_INIT[2],0x00, 0x00, 0x00), Word(RCON_INIT[3],0x00, 0x00, 0x00),
        Word(RCON_INIT[4],0x00, 0x00, 0x00), Word(RCON_INIT[5],0x00, 0x00, 0x00),
        Word(RCON_INIT[6],0x00, 0x00, 0x00), Word(RCON_INIT[7],0x00, 0x00, 0x00),
        Word(RCON_INIT[8],0x00, 0x00, 0x00), Word(RCON_INIT[9],0x00, 0x00, 0x00))
val NK_WORDS = 4
val NB_WORDS = 4
val NR = 10
...
fun decryptString(plainText: String, keyText: String) : String {
    val plainTextArray : Array<Int> = Array(16, {0})
    val keyTextArray: Array<Int> = Array(16, {0})
    if (plainText.length != 16) {
        throw RuntimeException("array length must be 16")
    }
    var tmpkeyText: String = keyText
    if (keyText.length  < 16) {
        tmpkeyText = keyText + "~~~~~~~~~~~~~~~~".substring(0, 16 - keyText.length)
    }
    if (keyText.length > 16) {
        tmpkeyText = keyText.substring(0, 16)
    }
    for (i in 0..15) {
        plainTextArray[i] = plainText[i].toInt() and 0xFF
    }
    for (i in 0..15) {
        keyTextArray[i] = tmpkeyText[i].toInt() and 0xFF
    }
    val result = decrypt(plainTextArray, keyTextArray).map { it -> it.toChar() }.toCharArray()
    return String(result).replace("~", "")
}

fun encryptString(plainText: String, keyText: String) : String {
    val plainTextArray : Array<Int> = Array(16, {0})
    val keyTextArray: Array<Int> = Array(16, {0})
    var tmpPlainText: String = plainText
    var tmpkeyText: String = keyText
    if (plainText.length  < 16) {
        tmpPlainText = plainText + "~~~~~~~~~~~~~~~~".substring(0, 16 - plainText.length)
    }
    if (plainText.length > 16) {
        tmpPlainText = plainText.substring(0, 16)
    }
    if (keyText.length  < 16) {
        tmpkeyText = keyText + "~~~~~~~~~~~~~~~~".substring(0, 16 - keyText.length)
    }
    ...
    for (i in 0..15) {
        plainTextArray[i] = tmpPlainText[i].toInt() and 0xFF
    }
    for (i in 0..15) {
        keyTextArray[i] = tmpkeyText[i].toInt() and 0xFF
    }
    val result = encrypt(plainTextArray, keyTextArray).map { it -> it.toChar() }.toCharArray()
